...
describe("自动更新 - 真实场景测试（调用真实代码）", () => {
  ...
  describe("更新链接验证", () => {
    ...
    it("latest.yml 应该在公开发布源存在，或明确表现为尚未发布 manifest", async () => {
      const url =
        "https://github.com/tianzecn/SkillsHub/releases/latest/download/latest.yml";
      const result = await checkUrlAccessible(url);

      console.log(`latest.yml 状态码: ${result.statusCode}`);
      if (!result.accessible) {
        console.warn(
          "警告: SkillsHub 当前 latest release 尚未提供 latest.yml，或当前环境无法访问 GitHub",
        );
        expect([0, 404]).toContain(result.statusCode);
        return;
      }
    ...
    it("Windows x64 安装包链接应该可以访问", async () => {
      ...
      const url = `https://github.com/tianzecn/SkillsHub/releases/download/v${version}/PromptHub-Setup-${version}-x64.exe`;
      const result = await checkUrlAccessible(url);
