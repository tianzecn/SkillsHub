...
Download the latest version v0.8.1 from [Releases](https://github.com/tianzecn/SkillsHub/releases):
...
- [x] **Installed Skill AI insights**: My Skills detail pages show the same structured AI insight panel above the skill description
- [x] **Store insight reuse on import**: Ready matching insights generated in the store are carried into the installed Skill to avoid duplicate token use
- [x] **Local-content fallback generation**: Installed Skills can still generate insights from the full local `SKILL.md` when no reusable cache exists
- [x] **Persistent insight cache**: AI insights are stored in local SQLite, reused after restart, and cleared when refreshed or deleted
...
### Latest Version v0.8.1 (2026-05-13) 🎉

**Persistent Skill Insight Cache**

- 🧠 **Local persisted cache**: AI insight results are saved to SQLite and reused after restarting PromptHub
- 🔁 **Cache cleanup on refresh/delete**: Refreshing or deleting an insight also clears its persisted record to avoid stale reuse
